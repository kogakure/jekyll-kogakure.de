(function($) {
  ...
  var LunrSearch = (function() {
    ...
    LunrSearch.prototype.search = function(query) {
      ...
      if (query.length < 2) {
        this.$results.removeClass("search-results--open")
        // this.$results.hide();
        this.$entries.empty();
      } else {
    ...
    LunrSearch.prototype.displayResults = function(entries) {
      ...
      if (entries.length === 0) {
        $entries.append('<p class="search-results__no-entries">Es wurden leider keine <span class="nobr">Suchergebnisse gefunden</span>.</p>');
      } else {
        $entries.append(this.template({entries: entries}));
      }

      $results.addClass("search-results--open");
      // $results.show();
    };
  ...
  $.fn.lunrSearch.defaults = {
    ...
    entries   : '.search-results__entries',         // selector for search entries containing element (contained within results above)
